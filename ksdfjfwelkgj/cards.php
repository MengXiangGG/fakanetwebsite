...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] == 'add') {
    ...
    if (empty($cards_text)) {
        ...
        foreach ($lines as $line_index => $card_line) {
            ...
            // 使用空格分隔，商品名称作为卡号，CDK作为密码
            $parts = preg_split('/\s+/', $card_line, 2);
            
            if (count($parts) == 2) {
                // 格式：商品名称 CDK
                $card_number = trim($parts[0]);    // 商品名称作为卡号
                $card_password = trim($parts[1]);  // CDK作为密码
            } else {
                // 如果只有1个部分，跳过这一行（不符合格式）
                continue;
            }
            
            // 检查卡号+密码组合是否已存在
            $check_stmt = $pdo->prepare("SELECT id FROM cards WHERE card_number = ? AND card_password = ? AND product_id = ?");
            $check_stmt->execute([$card_number, $card_password, $product_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['batch_action'])) {
    ...
    if (!empty($card_ids)) {
        ...
        $_SESSION['success'] = '批量删除成功！删除了 ' . count($card_ids) . ' 个卡密';
                            ...
                            <form method="POST">
                                ...
                                <div class="mb-3">
                                    <label class="form-label">
                                        卡密内容（从第二行开始导入，第一行作为标题会自动跳过）
                                    </label>
                                    <textarea name="cards" class="form-control" rows="12" placeholder="商品名称或标题（第一行会自动跳过）&#10;卡号1 密码1&#10;卡号2 密码2&#10;卡号3 密码3&#10;...（如果没有密码，可以只写卡号）" required><?php echo isset($_POST['cards']) ? htmlspecialchars($_POST['cards']) : ''; ?></textarea>
                                </div>
                                ...
                                <div class="card bg-light">
                                    ...
                                    <div class="card-body">
                                        <div class="card-example">
                                            复活币*2赠幸运福袋 1EFA96C8DB9E3306<br>
                                            复活币*2赠幸运福袋 D8F900F23D9E2A03<br>
                                        ...
                                        <div class="mt-2">
                                            <small class="text-muted">
                                                ...
                                                说明：第一行作为标题会自动跳过，从第二行开始导入。使用空格分隔卡号和密码，如果没有密码可以只写卡号
                                            </small>
                                        </div>
                                    </div>
                                </div>
...
<div class="batch-actions">
    <form method="POST" id="batchForm">
        ...
        <!-- 隐藏的卡密ID字段 -->
        <div id="batchCardIds" style="display: none;"></div>
    </form>
</div>
                            <div class="table-responsive">
                                <table class="table table-striped table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr class="<?php echo $card['status'] ? 'card-sold' : ''; ?>">
                                            <td>
                                                <?php if (!$card['status']): ?>
...
<script>
    ...
    function updateBatchForm() {
        ...
        const batchContainer = document.getElementById('batchCardIds');
        ...
        selectedCards.forEach(checkbox => {
            ...
            hiddenInput.name = 'card_ids[]';
            hiddenInput.value = checkbox.value;
            batchContainer.appendChild(hiddenInput);
        });
    }
